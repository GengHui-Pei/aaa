...
import random
import math
from ..prep_ambhand_data import random_bg_img
...
def augmentation(img, bbox, joint_coord, joint_valid, hand_type, mode, joint_type, bg_idx=None):
    img = img.copy();
...
def refine_transform_input_to_output_space(joint_coord):
    # transform to output heatmap space
    # joint_coord = joint_coord.copy()

    joint_coord[:,0] = joint_coord[:,0] / cfg.input_img_shape[1] * cfg.output_hm_shape[2]
    joint_coord[:,1] = joint_coord[:,1] / cfg.input_img_shape[0] * cfg.output_hm_shape[1]
    joint_coord[:,2] = joint_coord[:, 2] - joint_coord[20, 2]
...
def process_bbox(bbox,img_size=None):
