...
from common.utils.preprocessing import load_img, load_skeleton, process_bbox, get_aug_config, augmentation, \
...
from common.kvision import kprint,draw1_2d_skeleton,draw_2d_skeleton,skeleton_inter2mano,skeleton_mano2inter
from common.prep_ambhand_data import get_bbox, cam2pix,random_bg_img
import copy
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, transform, mode, annot_subset=None):
        ...
        self.skeleton2sequence = np.load(f'{cfg.root_dir}/dataset/syn/rs1_skeleton_to_sequence_256.npy',allow_pickle=True)
        self.datalist = []
    ...
    def __getitem__(self, idx):
        ...
        #print('bbb',idx,org_joint_coord[0],org_joint_coord[24])
        ...
        #print('aaa',idx,joint_coord_crop[0],joint_coord_crop[24])
       #经过augmentation，joint_coord_crop和org_joint_coord是不一样的
        ...
        if self.mode == 'KMean':
            return joint_coord_crop_space[:21]
...
if __name__ == '__main__':
    ...
    matplotlib.use('Agg')
    import os.path as osp
    from common.utils.preprocessing import load_img, load_skeleton, process_bbox, transform_input_to_output_space, trans_point2d
    from common.utils.vis import vis_keypoints, vis_3d_keypoints
    from net.main_net import heatmap2joint, joint2heatmap
    ...
    for i in range(3):
        ...
        for itr, (inputs, targets, meta_info) in enumerate(batch_generator):
            ...
            # joint_km = heatmap2joint(joint2heatmap(joint_km.cuda()))

            preds['joint_coord'].append(joint_km.cpu().numpy())
