...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, transform, mode):
        ...
        self.img_path = '/root/Source/jyk/InterHand2.6M/images'
        self.annot_path = '/root/Source/jyk/InterHand2.6M/annotations'

        if self.mode == 'val':
            # self.rootnet_output_path = '../data/InterHand2.6M/rootnet_output/rootnet_interhand2.6m_output_val.json'
            self.rootnet_output_path = '/root/Source/jyk/InterHand2.6M/rootnet_output/rootnet_interhand2.6m_output_all_val.json'
        else:
            self.rootnet_output_path = '/root/Source/jyk/InterHand2.6M/rootnet_output/rootnet_interhand2.6m_output_all_test.json'
        ...
        self.skeleton2sequence = np.load(f'{cfg.root_dir}/net/rs1_skeleton_to_sequence_256_i.npy', allow_pickle=True)
        ...
        db = COCO(osp.join(self.annot_path, 'all', 'InterHand2.6M_' + self.mode + '_data.json'))
        ...
        #     joints = json.load(f)
        with open(osp.join(self.annot_path, 'all', 'InterHand2.6M_' + self.mode + '_camera.json')) as f:
            cameras = json.load(f)
        with open(osp.join(self.annot_path, 'all', 'InterHand2.6M_' + self.mode + '_joint_3d.json')) as f:
            joints = json.load(f)
        ...
        #self.datalist = self.datalist_sh[:10]
        self.datalist = random.choices(self.datalist_sh, k=10)
    ...
    def __getitem__(self, idx):
        ...
        joint_sequence = ((joint_coord[:21, None, :] - self.skeleton2sequence[None, :, :]) ** 2).sum(
            -1).argmin(1)
    ...
    def evaluate(self, preds):
        ...
        for n in range(sample_num):
            ...
            #print(pred_joint_coord_cam.shape)   42,3
            #print(gt_joint_coord.shape)        42,3
            ...
            vis = False
            if vis:
                filename = addr+'/out_' + str(n) + '_3d.jpg'
        ...
        print(eval_summary)
        print('MPJPE for single hand sequences: %.2f' % (np.mean(mpjpe_sh[:21])))
        print()
